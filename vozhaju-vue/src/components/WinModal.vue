...
<template>
  <transition name="modal-fade">
    <div v-if="show" class="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div class="bg-orange-50 border-4 border-amber-900 rounded-3xl p-8 max-w-sm w-full text-center shadow-2xl transform transition-transform">
        <h2 class="text-3xl font-black text-amber-900 mb-2">{{ title }}</h2>
        
        <!-- Stars Display -->
        <div v-if="stars > 0" class="flex justify-center gap-2 mb-4">
        ...
        <p class="text-lg text-amber-800 font-bold mb-4">{{ message }}</p>

        <!-- Rewards Display -->
        <div v-if="coinsEarned > 0 || scoreEarned > 0" class="flex justify-center gap-4 mb-8 bg-amber-100 p-3 rounded-2xl border-2 border-amber-300">
        ...
        <button 
          @click="emit('close')"
          class="w-full py-3 bg-green-500 hover:bg-green-600 text-white rounded-xl font-bold text-xl border-b-4 border-green-700 active:border-b-0 active:translate-y-1 transition-all">
          {{ buttonText }}
        </button>
      </div>
    </div>
